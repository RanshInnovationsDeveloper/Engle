...
const addToRemember = async (req, res) => {
    try {
        ...
        res.status(200).json({ status: "Item added to Remembers" });
    } catch (error) {
        console.log(error);
        res.status(500).json({ status: "Error", message: error.message });
    }
...
const removeFromRemember = async (req, res) => {
    ...
    try {
        ...
        if (docSnap.exists()) {
            ...
            res.status(200).json({ status: "Item removed from Remembers" });
        }
    } catch (error) {
        console.log(error);
    }
...
const fetchAllRememberItems = async (req, res) => {
    try {
        ...
        const docRef = doc(db, "remember", userId);
        const docSnap = await getDoc(docRef);
        //Check if Remember exists
        if (!docSnap.exists()) {
            await setDoc(docRef, { userId });
            return res
                .status(200)
                .json({ status: "No Remember data found for this user" });
        }
